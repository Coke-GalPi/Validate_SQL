...
select 
    ...
    (sum(hecho_vuelos.time_delay) - lag(sum(hecho_vuelos.time_delay)) over (order by years.fl_year))
        / cast(lag(sum(hecho_vuelos.time_delay)) over (order by years.fl_year) as decimal(10, 2)) as growth_rate1
